...
export function PaginationPrevious({
  ...
  children = 'Previous',
  scroll = false,
}: React.PropsWithChildren<{ href?: string | null; className?: string; scroll?: boolean }>) {
  return (
    <span className={clsx(className, 'grow basis-0')}>
      <Button
        {...(href === null ? { disabled: true } : { href, scroll })}
...
export function PaginationPage({
  ...
  children,
  scroll = false,
}: React.PropsWithChildren<{ href: string; className?: string; current?: boolean; scroll?: boolean }>) {
  return (
    <Button
      href={href}
      scroll={scroll}
