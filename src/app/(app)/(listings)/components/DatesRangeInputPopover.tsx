...
interface Props {
  className?: string
  defaultDateValue?: [Date | null, Date | null]
...
const DatesRangeInputPopover: FC<Props> = ({
  ...
  const [startDate, setStartDate] = useState<Date | null>(defaultDateValue?.[0] || new Date())
  const [endDate, setEndDate] = useState<Date | null>(defaultDateValue?.[1] || new Date(Date.now() + 3 * 24 * 60 * 60 * 1000))
  //
